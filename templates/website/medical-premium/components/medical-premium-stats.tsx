...
function StatValue({ value }: { value: string }) {
  ...
  return (
    <dd ref={ref} className="mp-metric">
      {display}
    </dd>
  );
}

export function MedicalPremiumStats({
  stats = DEFAULT_STATS,
}: MedicalPremiumStatsProps) {
  if (!stats.length) return null;

  return (
    <section
      id="stats"
      data-v2-component="medical-premium-stats"
      aria-labelledby="mp-stats-title"
      className="mp-reveal mp-section-band flex min-h-[50vh] items-center py-20 sm:py-28"
    >
      <div className="mp-container">
        <h2 id="mp-stats-title" className="sr-only">
          Key metrics
        </h2>
        <dl className="mp-reveal-stagger grid gap-8 sm:grid-cols-2 lg:grid-cols-4 lg:gap-0 lg:divide-x lg:divide-[var(--border-subtle)]">
          {stats.map((stat) => (
            <div key={`${stat.label}-${stat.value}`} className="px-4 text-center lg:px-8">
              <StatValue value={stat.value} />
              <dt className="mp-stat-label mt-3">{stat.label}</dt>
              {stat.detail ? <p className="mp-caption mt-1">{stat.detail}</p> : null}
            </div>
          ))}
        </dl>
      </div>
