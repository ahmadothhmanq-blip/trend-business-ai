/**
 * Idempotent website commit replay store.
 */

import type { SupabaseClient } from "@supabase/supabase-js";
import type { IdempotencyCachedResponse } from "@/lib/website/platform/types";

export const WEBSITE_COMMIT_IDEMPOTENCY_TTL_DAYS = 7;
...
/**
 * Remove expired idempotency rows (best-effort; safe when column/table missing).
 */
export async function purgeExpiredIdempotentCommits(
  supabase: SupabaseClient,
): Promise<number> {
  const now = new Date().toISOString();
  const { data, error } = await supabase
    .from("website_commit_idempotency")
    .delete()
    .lt("expires_at", now)
    .select("id");
...
export async function findIdempotentCommit(
  supabase: SupabaseClient,
  params: {
    userId: string;
    generationId: string;
...
export async function storeIdempotentCommit(
  supabase: SupabaseClient,
  params: {
    userId: string;
    generationId: string;
  ...
  expiresAt.setDate(expiresAt.getDate() + WEBSITE_COMMIT_IDEMPOTENCY_TTL_DAYS);
  ...
  const { error } = await supabase.from("website_commit_idempotency").insert(row);
  ...
  try {
    await purgeExpiredIdempotentCommits(supabase);
  } catch {
    // Non-fatal cleanup.
  }
}
