...
- Use DB-backed signed cookie-session auth in lib/auth.ts via getSession(). Session must be { sessionId: string; userId: string; email: string; role: string }. Never use session.user — use session.email / session.userId / session.sessionId / session.role. Export isStaffRole. Login/signup must verify/hash passwords and issue a signed session cookie via signSessionCookie (lib/session-cookie.ts). Middleware must call verifySessionCookie — never trust cookie presence or unsigned session_role alone. getSession must verify the signed cookie then load the DB Session row. CRUD APIs must scope by ownerId and enforce staff vs owner with 403. Do not scaffold NextAuth CredentialsProvider in lib/auth.ts.
