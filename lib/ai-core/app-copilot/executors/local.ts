/**
 * Local executor — structured app model mutations via runAppAssistantAgent patterns.
 */
...
export type AppLocalCopilotSuccess = {
  ok: true;
...
export async function executeLocalAppCopilotCommand(params: {
...
> {
  ...
  if (!assistantResult.applied || !assistantResult.model) {
    return {
      ...
      error:
        ...
        "No structured edit matched. Try one of the example commands.",
    };
  }
  ...
  const committed = await commitAppBlueprintRevision({
    ...
    commit: {
      ...
      mutationMeta: { capability: params.capability, tier: "local" },
    },
  });
  ...
  return {
    ...
    summary: assistantResult.actions.join(" · ") || "App updated.",
    assistantResult,
