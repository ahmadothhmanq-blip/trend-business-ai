...
async function profileStage<T>(
  ...
  return profiler.measure(category, label, fn, { layer: label });
}
...
export class LayerRunner {
  async run<TGeneration = unknown, TFinal = unknown>(
    ...
    let artifacts: CoreLayerArtifacts = {
      brief,
    ...
    // Template selection:
    // Products may implement prepareTemplate (Website Builder: MAOE + PRE + templates).
    // Other products use the Industry Template Engine (Phase 6).
    emit(progress, onProgress, "template", "Selecting template...");
    if (adapter.prepareTemplate) {
      await profileStage("layer-runner", "template-selection", async () => {
        const prepared = await adapter.prepareTemplate!(
          brief,
          artifacts,
          { onProgress },
        );
        brief = prepared.brief;
        artifacts.brief = prepared.brief;
        if (prepared.artifacts.templateSelection !== undefined) {
          artifacts.templateSelection = prepared.artifacts.templateSelection;
        }
        layersExecuted.push(...(prepared.layersExecuted ?? ["template"]));
      });
    } else {
      await profileStage("layer-runner", "template-selection", async () => {
    ...
    if (adapter.layers.idea && adapter.runIdea) {
      ...
      artifacts.businessProfile = await profileStage("ai-planning", "idea", () =>
        adapter.runIdea!(brief, ctx, artifacts),
      );
    ...
    if (adapter.layers.strategy && adapter.runStrategy) {
      ...
      artifacts.strategy = await profileStage("ai-planning", "strategy", () =>
        adapter.runStrategy!(brief, artifacts.businessProfile!, ctx, artifacts),
      );
    ...
    if (adapter.layers.design && adapter.runDesign) {
      ...
      artifacts.designSystem = await profileStage("ai-planning", "design", () =>
        adapter.runDesign!(
          brief,
    ...
    if (adapter.layers.assets && adapter.runAssets) {
      ...
      artifacts.assetManifest = await profileStage("image-generation", "assets", () =>
        adapter.runAssets!(brief, artifacts, ctx),
      );
      layersExecuted.push("assets");
    }

    emit(progress, onProgress, "generation", "Generating product output...");
    const generation = await profileStage("plugin", "generate", () =>
      adapter.runGeneration(brief, artifacts, ctx),
    );
    ...
    if (adapter.layers.quality && adapter.runQuality) {
      ...
      artifacts.qualityReport = await profileStage("validation", "quality", () =>
        adapter.runQuality!(brief, artifacts, generation, ctx),
      );
    ...
    if (adapter.layers.seo) {
      ...
      if (adapter.runSeo) {
        artifacts.seoPackage = await profileStage("seo", "seo-package", () =>
          adapter.runSeo!(brief, artifacts, generation, ctx),
        );
    ...
    if (adapter.layers.performance) {
      ...
      if (adapter.runPerformance) {
        artifacts.performanceReport = await profileStage(
          "validation",
          "performance-checks",
          () => adapter.runPerformance!(brief, artifacts, generation, ctx),
        );
      } else {
    ...
    if (adapter.layers.finalize && adapter.finalize) {
      ...
      finalOutput = await profileStage("build-preparation", "finalize", () =>
        adapter.finalize!(brief, artifacts, generation, ctx),
      );
    ...
    if (adapter.completeRun) {
      const completed = await adapter.completeRun(brief, artifacts, { onProgress });
