/**
 * Internal quality dimension checks — used by QASHE build-spec (no QASHE import).
 */
...
export type BuildAutoQualityReportInternalInput = {
...
function checkDesignConsistency(params: {
  ...
  if (
    ...
    !content
      .toLowerCase()
      .includes(designSystem.typography.headingFont.toLowerCase().slice(0, 8)) &&
...
/**
 * Build dimension-level quality report (structure, brand, SEO, media, performance).
 * Called by QASHE build-spec — must not import QASHE to avoid circular deps.
 */
export function buildAutoQualityReportInternal(
  input: BuildAutoQualityReportInternalInput,
  ...
  return {
    passed:
      !structureFailed &&
      (base?.passed ?? dimensions.every((d) => d.passed || d.name !== "structure")),
    dimensions,
    weakSections,
