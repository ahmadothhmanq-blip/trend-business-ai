...
/**
 * POST /api/website-builder/[id]/theme
 * Full website design rebuild — layout, hero, nav, sections, UI components.
 * Business identity (content, images, SEO, business info) is preserved.
 */
  ...
  try {
    ...
    const saved = await updateWebsiteGenerationInPlace({
      ...
      project: {
        ...switched.project,
        settings: {
          ...(switched.project.settings ?? {}),
          websiteThemeId:
            parsed.data.themeId || parsed.data.templateIntelligenceId,
        },
