...
import { splitEditorScene } from "@/lib/ai-core/video-production-platform/editor-mvp/service";
...
export async function POST(request: Request, context: Params) {
  ...
  try {
    const result = await splitEditorScene(auth.supabase, {
      userId: auth.user!.id,
      projectId: projectParsed.id,
      sceneId: sceneParsed.id,
      atSec: valid.data.atSec,
      requestId: valid.data.requestId,
    });
    return NextResponse.json(result);
  } catch (error) {
    return editorMvpErrorResponse(error) || serverErrorResponse("video-editor.split", error);
  }
}
