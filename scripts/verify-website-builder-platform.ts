...
import { PLATFORM_SECTION_COMPONENTS } from "@/lib/website-builder-platform/contracts/component-registry";
...
async function verifyIndustries() {
  ...
  for (const prompt of INDUSTRY_PROMPTS) {
    ...
    results.push({
      ...
      services: pack.content.services.length,
...
async function main() {
  ...
  for (const row of industries) {
    console.log(
      `  ✓ ${row.industry.padEnd(22)} routing=${row.routing.padEnd(16)} services=${row.services} images=${row.images}`,
    );
  }
