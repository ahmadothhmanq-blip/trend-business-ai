...
export function IncidentsPanel() {
  ...
  const create = async () => {
    ...
    setTitle(""); load(); toast.success(wt("toasts.incidentOpened"));
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">
        <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder={wt("forms.incidentTitle")} className={`${dashboardInputClass} max-w-xs`} />
        <Button onClick={() => void create()}>{wt("panels.incidents.openIncident")}</Button>
      </div>
        ...
        {incidents.map((i) => (
          <li key={i.id} className="flex items-center justify-between rounded-lg border border-white/5 px-3 py-2">
            ...
            {i.status !== "resolved" && i.status !== "closed" && (
              <Button size="sm" variant="outline" onClick={() => void resolve(i.id)}>{wt("panels.incidents.resolve")}</Button>
            )}
          </li>
        ))}
      </ul>
