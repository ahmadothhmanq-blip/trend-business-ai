...
export function ThreatIntelligencePanel() {
  ...
  const addThreat = async () => {
    ...
    setTitle(""); load(); toast.success(wt("toasts.threatAdded"));
  };
  ...
  const addIoc = async () => {
    ...
    if (!res.ok) return toast.error(wt("toasts.failed"));
    setIocValue(""); load(); toast.success(wt("toasts.iocAdded"));
  };

  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <div className="space-y-3 rounded-xl border border-white/[0.08] bg-white/[0.02] p-4">
        <p className="text-xs uppercase text-white/40">{wt("panels.threats.title")}</p>
        <div className="flex gap-2">
          <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder={wt("forms.threatTitle")} className={dashboardInputClass} />
          <Button onClick={() => void addThreat()}>{wt("panels.threats.add")}</Button>
        </div>
        <ul className="space-y-1 text-sm text-white/60">
          {threats.map((t) => <li key={t.id}>{t.title} · {t.severity} · {t.status}</li>)}
          {threats.length === 0 && <li className="text-white/30">{wt("panels.threats.noThreats")}</li>}
        </ul>
      </div>
      <div className="space-y-3 rounded-xl border border-white/[0.08] bg-white/[0.02] p-4">
        <p className="text-xs uppercase text-white/40">{wt("panels.threats.iocs")}</p>
        <div className="flex gap-2">
          <Input value={iocValue} onChange={(e) => setIocValue(e.target.value)} placeholder={wt("forms.iocValue")} className={dashboardInputClass} />
          <Button onClick={() => void addIoc()}>{wt("panels.threats.addIoc")}</Button>
        </div>
        <ul className="space-y-1 text-sm text-white/60">
          {iocs.map((i) => <li key={i.id}>{i.ioc_type}: {i.value}</li>)}
          {iocs.length === 0 && <li className="text-white/30">{wt("panels.threats.noIocs")}</li>}
        </ul>
      </div>
    </div>
  );
