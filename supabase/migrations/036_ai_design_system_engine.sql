...
-- design_presets, design_systems, generated_designs
...
create table if not exists public.design_systems (
  ...
  preset_id text references public.design_presets (id) on delete set null,
  template_id text,
  name text not null,
  style text not null,
...
-- Align SaaS smart template with Tech design preset (code catalog + DB).
-- Safe when Smart Templates migration has not been applied yet.
do $tag$
begin
